...
def smooth(x, **vars_dict):
    times = np.arange(0, len(x) / vars_dict["smth_window"], 1) * vars_dict["time_base"] * vars_dict["smth_window"]
    smoothed = np.mean(x.reshape(-1, vars_dict["smth_window"]), axis=-1)
...
def fit_waiting_times(tunnel, **vars_dict):
    waiting_times = np.diff(tunnel.nonzero()[0]) * vars_dict["time_base"] * vars_dict["smth_window"]
    hist, bins = np.histogram(waiting_times,
                              bins=np.logspace(np.log10(vars_dict["time_base"] * vars_dict["smth_window"]),
                                               np.log10(.0015 * vars_dict["smth_window"]), 30))
...
def fit_psd(jumps, p0=[1e-5, 1e2, 1e5, 0.3, 0.4, 0.25], **vars_dict):
    fs = 1 / vars_dict["time_base"]

    f, Pxx_den = periodogram(jumps, fs, scaling="density")

    psd_popt, psd_pcov = curve_fit(
        lambda x, a, b, c, d, e, f: np.log10(psd_ffunc(freq=x, t_exp=a, gamma_rts_1=b, gamma_rts_2=c,
